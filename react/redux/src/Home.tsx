...
import { BENCHMARK_SIZE, BENCHMARK_TOGGLE_SIZE, evaluate, shortId, type Todo } from '@anchor-benchmark/shared';
import { type FormEvent, memo, useEffect, useRef, useState } from 'react';
import { useAppDispatch, useAppSelector } from './store';
import { addTodo, deleteTodo, selectTodos, toggleTodo, toSerializableTodo } from './todosSlice';
...
const TodoApp = () => {
  ...
  const handleAddTodo = (e: FormEvent) => {
    ...
    if (newTitle.trim()) {
      ...
      dispatch(addTodo(toSerializableTodo(newTodo)));
      setNewTitle('');
    }
  };

  const handleAddTodoBenchmark = () => {
    return evaluate(() => {
      const newTodo: Todo = {
        id: shortId(),
  ...
  const handleToggleTodoBenchmark = (id: string) => {
    return evaluate(() => {
    ...
    <div ref={ref} className="bg-slate-900 rounded-2xl shadow-xl max-w-md w-full mx-4 flex flex-col gap-4">
      ...
      <div className="px-4 max-h-[480px] overflow-y-auto">
        <TodoList
          ...
          toggleTodoBenchmark={handleToggleTodoBenchmark}
      ...
      <li ref={ref} className="flex items-center gap-2">
        <div className="flex items-center flex-1 gap-3 bg-slate-800/70 p-2 rounded-md">
          <label className="text-slate-300">
            <input type="checkbox" checked={item.completed} onChange={() => toggleTodo(item.id)} className="sr-only" />
            {item.completed ? (
