...
import { type FormEvent, memo, useEffect, useRef, useState, useMemo } from 'react';
  ...
  return (
    <div ref={ref} className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full mx-4">
      <div className="text-center">
        ...
        <div className="text-sm text-gray-500">Using React's built-in state management</div>
      </div>
    </div>
  );
};
...
const TodoApp = () => {
  ...
  const [todoItems, setTodoItems] = useState<Todo[]>(initialTodoItems);
  
  // Derived stats - computed from todoItems
  ...
  const benchmarkAdd = () => {
    ...
    const addNext = () => {
      if (count < BENCHMARK_SIZE) {
        ...
        const newTodo: Todo = {
          id: shortId(),
          title: `New Todo (${todoItems.length + count + 1})`,
          completed: false,
          createdAt: new Date(),
          updatedAt: new Date(),
          priority: 'medium',
...
const TodoStats = ({ stats }: { stats: { total: number; completed: number; active: number; } }) => {
  const ref = useRef<HTMLDivElement>(null);
